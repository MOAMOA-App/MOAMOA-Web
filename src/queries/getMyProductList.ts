...
export const getMyProductList = async (config?: AxiosRequestConfig) => {
...
export const useGetMyProductList = (configOptions?: QueryConfigOptions) => {
    const info = useQuery<Response, AxiosError, Goods[]>({
        queryKey: QUERY_KEY,
        queryFn: () => getMyProductList(configOptions?.config),
